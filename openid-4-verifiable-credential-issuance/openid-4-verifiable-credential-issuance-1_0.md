...
* `client_metadata`: OPTIONAL. A JSON object containing the Client Metadata values. It MUST be UTF-8 encoded. It MUST NOT be present if `client_metadata_uri` parameter is present.
* `client_metadata_uri`: OPTIONL. A string containing an HTTPS URL pointing to a resource where a JSON object with the Client Metadata can be retrieved. The scheme used in the `client_metadata_uri` value MUST be https. The `client_metadata_uri` value MUST be reachable by the Credential Issuer. It MUST NOT be present if `client_metadata` parameter is present.
...
If the Client requested encrypted responses, the Credential Issuer MUST select an encryption algorithm based on the algorithms supported by the Client and encode the claims in the Credential Reponse as a JWT. In this case, the media type of the reponse MUST bet set to `application/jwt`. If encryption was negotiated with the Credential Issuer at registration time or in the Credential Request and the Credential Response is not encrypted, the Client SHOULD reject the Credential Response.
...
* `credential_encrypted_response_alg`: OPTIONAL. JWE [@!RFC7516] `alg` algorithm [@!RFC7518] REQUIRED for encrypting Credential and/or Batch Credential Responses. The default, if omitted, is that no encryption is performed.
* `credential_encrypted_response_enc`: OPTIONAL. JWE [@!RFC7516] enc algorithm [@!RFC7518] REQUIRED for encrypting UserInfo Responses. If userinfo_encrypted_response_alg is specified, the default for this value is A128CBC-HS256. When userinfo_encrypted_response_enc is included, userinfo_encrypted_response_alg MUST also be provided.
