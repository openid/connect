...
The End-User may desire to use the same Subject identifier value across devices. This is not possible unless there is a way to sync a private key used to self-sign the Self-Issued ID Token across devices. This will not be possible if the private key is hardware bound and is not exportable.
