...
Note: A request MUST contain a `presentation_definition` or a `presentation_definition_uri` or a scope value `openid_presentation:<credential_type>` but the three elements are mutually exclusive.
...
A verifiable presentation can be requested by adding a scope value of the form `openid_presentation:<credential_type>` to the `scope` request parameter. Such a scope value is a default presentation request, i.e. it is resolved by the authorization server to a presentation definition including one input descriptor for every format for that particular abstract credential type the wallet supports. 

Such a (virtual) presentation definition for a wallet supporting a single format only would look like this: 
...
If the wallet supports multiple formats for a given abstrect credential type, the (virtual) presentation definition would contain multiple input descriptors (one per format) and a presentation_requirement requesting presentation of at most one of the credentials.
...
```JSON
{
    ...
    "input_descriptors": [
        {
            "id": "scope_<credential_type>_ldp",
            "group": [
                "A"
            ],
            "format": {
